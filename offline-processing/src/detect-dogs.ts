  ...
  async function worker() {
    while (true) {
      ...
      try {
        ...
        status(
          `${processed + failed} / ${unprocessed.length} photos · ${totalDogsFound} dogs · ${(elapsed / processed).toFixed(1)} s/img · ETA ${fmtDuration((unprocessed.length - (processed + failed)) * (elapsed / processed))}`,
        );
      } catch (err) {
        failed++;
        consecutiveFailures++;
