...
function getMode(): Mode {
  const value = process.env.INGEST_MODE?.toLowerCase();
...
async function main() {
  const mode = getMode();
  const rawSourceDir = process.env.SOURCE_DIR;
  const sourceDir = rawSourceDir?.startsWith('~')
    ? path.join(os.homedir(), rawSourceDir.slice(1))
    : rawSourceDir;
  const sshHost = process.env.SSH_HOST;
  const sshDestDir = process.env.SSH_DEST_DIR;

  if (!sourceDir) {
    console.error('Missing required env var: SOURCE_DIR');
    process.exit(1);
  }
